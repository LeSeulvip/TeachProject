$(function() {
  ...
  $('#btnAjax').click(function() {
    //加载本地存储中的serverToken
    var severToken = localStorage.getItem('serverToken');

    //nginx反向代理http:127.0.0.1:20000/为/dataService/
    //所以这两个请求完全等效,但是/dataService/没有配置跨域,可以避免跨域攻击
    $.ajax({
      url: '/dataService/',
      //将本地存储的token回发到服务器
      data: { echo: $('#txtEcho').val(), token: severToken },
      dataType: 'json'
    }).done(function(data) {
      console.log(data);
      //把服务器的token保存到本地存储
